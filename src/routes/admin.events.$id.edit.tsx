...
const getEvent = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    if (!import.meta.env["VITE_SUPABASE_URL"] || !import.meta.env["SUPABASE_SERVICE_ROLE_KEY"]) {
      return null as unknown as EventRow;
    }
...
function eventToForm(event: EventRow): FormState {
  return {
    title: event.title,
    ...
    <AdminLayout>
      <div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <Tag tone="purple">Edit</Tag>
        </div>
        <h1 className="mt-4 text-3xl font-extrabold tracking-tight sm:text-4xl">Edit Event</h1>
        <p className="mt-2 text-muted-foreground">Update "{event.title}".</p>
      </div>
